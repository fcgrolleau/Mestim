...
This implies that the $p$-dimentional M-estimator $\hat{\theta}$ is an asymptotically normal, $\sqrt{n}$-consistent, estimator for $T(F)$.
...
Let's generate syntetic data with two predictors and a binary outcome such that $Z=(X_1,X_2,Y)^T$.
<details>
...
res_boot <- boot::boot(dat, boot_fun, R=9)
...
To obtain the last element of $\psi$, we need to do algebraic manipulations. Denoting $\mathcal{C}_{\tilde{d}, i}=\prod_{t=1}^2\mathbf{1}\big({\tilde{d}_t(S_{t,i})=A_{t,i}}\big)$ for simplicity, after substitution for $\hat{e}_1(S_1;\boldsymbol{\hat{\delta}})$ and $\hat{e}_2(S_2,A_1,S_1;\boldsymbol{\hat{\phi}})$, equation \@ref(eq:label) yields 
\begin{equation}
